...
namespace :puma do
  ...
  task :start do
    ...
    run "cd #{current_path} && RAILS_ENV=#{stage} bundle exec puma -d -C ./config/puma.rb"
  end

  desc "Stop the application"
  task :stop do
    # run "cd #{current_path} && RAILS_ENV=#{stage} && bundle exec pumactl -S #{puma_state} stop"
    run "kill -s SIGTERM `cat #{current_path}/tmp/pids/puma.pid`"
  end

  desc "Restart the application"
  task :restart, :roles => :app, :except => { :no_release => true } do
    # run "cd #{current_path} && RAILS_ENV=#{stage} && bundle exec pumactl -S #{puma_state} restart"
    run "kill -s SIGUSR2 `cat #{current_path}/tmp/pids/puma.pid`"
  end
